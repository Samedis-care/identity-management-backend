...
#     rails g auto_swagger_spec v1_app_roles
...
describe 'Roles API', swagger_doc: 'v1/swagger.json', "roles" => true  do
  ...
  path '/api/v1/apps/{app_id}/roles' do
...
parameter name: "id", in: :path,
      ...
      parameter name: :data, in: :body,
                schema: serializer::Schema.new.rswag_schema(
                  base_key: :data,
                  only: [:title, :description, :functionality_ids, :title_translations, :description_translations],
                  #required: []
                )
  ...
  path '/api/v1/apps/{app_id}/roles/{id}' do
...
parameter name: "id", in: :path,
    ...
    delete 'Delete role' do
      metadata[:operation]['x-candos'] = ["~/apps.admin+identity-management/roles.deleter"]
      metadata[:operation]['x-record-type'] = 'role'
      description <<~EOM
        To use this endpoint the current user needs to be authorized for the tenant to do any of these
