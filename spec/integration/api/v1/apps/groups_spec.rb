...
#     rails g auto_swagger_spec v1_app_groups
...
describe 'Actors::Groups API', swagger_doc: 'v1/swagger.json', "actors::groups" => true  do
  ...
  path '/api/v1/apps/{app_id}/groups' do

    get 'List Groups' do
      metadata[:operation]['x-candos'] = ["~/apps.admin+identity-management/actors.reader"]
      metadata[:operation]['x-record-type'] = 'group_actor'
      description <<~EOM
  ...
  path '/api/v1/apps/{app_id}/groups/{id}' do
