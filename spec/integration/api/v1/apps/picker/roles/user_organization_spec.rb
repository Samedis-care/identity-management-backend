...
#     rails g auto_swagger_spec v1_app_picker_role_user_organization_index
...
describe 'Actors API', swagger_doc: 'v1/swagger.json', "actors" => true  do
  ...
  path '/api/v1/apps/{app_id}/picker/roles/{role_id}/user_organization' do

    get 'List Actors' do
      metadata[:operation]['x-candos'] = ["~/apps.admin+identity-management/actors.reader+identity-management/roles.reader"]
      metadata[:operation]['x-record-type'] = 'picker_role_actor'
      description <<~EOM
