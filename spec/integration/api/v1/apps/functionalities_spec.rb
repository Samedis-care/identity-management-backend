...
#     rails g auto_swagger_spec v1_app_functionalities
...
describe 'Functionalities API', swagger_doc: 'v1/swagger.json', "functionalities" => true  do
  ...
  path '/api/v1/apps/{app_id}/functionalities' do

    get 'List Functionalities' do
      metadata[:operation]['x-candos'] = ["~/apps.admin+identity-management/functionalities.reader"]
    ...
    post 'Create Functionality' do
      metadata[:operation]['x-candos'] = ["~/apps.admin+identity-management/functionalities.writer"]
      metadata[:operation]['x-record-type'] = 'functionality'
      description <<~EOM
        To use this endpoint the current user needs to be authorized for the tenant to do any of these
...
parameter name: "id", in: :path,
      ...
      parameter name: :data, in: :body,
                schema: serializer::Schema.new.rswag_schema(
                  base_key: :data,
                  only: [:title, :description, :module, :ident, :role_ids, :title_translations, :description_translations],
                  #required: []
                )
  ...
  path '/api/v1/apps/{app_id}/functionalities/{id}' do
...
parameter name: "id", in: :path,
      ...
      response '200', 'Functionality updated' do
        schema serializer::Schema.new.swagger_schema single: true, meta: true, links: true
        run_test!
      end
    end
    ...
    delete 'Delete Functionality' do
      metadata[:operation]['x-candos'] = ["~/apps.admin+identity-management/functionalities.deleter"]
      metadata[:operation]['x-record-type'] = 'functionality'
      description <<~EOM
        To use this endpoint the current user needs to be authorized for the tenant to do any of these
