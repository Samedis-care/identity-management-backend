...
Rails.application.config.middleware.use OmniAuth::Builder do
  ...
  #   scope: "email",
  #   provider_ignores_state: true, # necessary to use state or CSRF error gets triggered
  #   client_options: { # use newer api endpoint that supports state
...
Rails.application.reloader.to_prepare do
  ...
  Devise.setup do |config|
    ...
    # config.secret_key = '4f0e361fcf44e78b126193e4ca4a9c3c1714fafb7a541019702753f3bbf04beda23d7092aaea597bbc7b9f7911a7f69e36e59367540cc753db0e5e8292739900'
    ...
    # config.parent_controller = 'ApiApplicationController'
    ...
    # config.pepper = '52a4eb3e944502015b8db8d6c06c157be2085f5451d896f47c91f8a687626af3d936788bd75138ccefba7359c9915ef65ac1151a76ffd2aeca211cc183fd9b9f'
...
# and resend the confirmation instructions (normally only during create)
module Devise
  module Models
    module Registerable
      module ClassMethods
        ...
        def new_with_session(params, session)
          # instead of just `new(params)`
          # also try to find an existing unconfirmed user
          _params = params.with_indifferent_access
          _user = where(email: _params[:email].to_s.downcase, confirmed_at: nil).first_or_initialize(_params)
          if _user.persisted? # it already existed
            # update fields
            _user.attributes = _params.except :password, :password_confirmation

            # update password in case it's different from first registration attempt
            if _params[:password].eql?(_params[:password_confirmation])
              # set latest password and avoid changed password email
              _user.set_password = _params[:password]
            end

            # reset the fields to allow resending the confirmation mail
            _user.confirmation_sent_at = nil
            _user.confirmation_token = nil
          end
          _user
        end
    ...
    class Token
      class << self
        def from_bearer_param(request)
          request.parameters[:bearer_token] || request.parameters[:bearer]
        end
      end
    end
  end
