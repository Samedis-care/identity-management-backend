namespace :openapi_spec do
  require 'rails/generators'
  require_relative '../generators/openapi_spec/openapi_spec_generator'
  ...
  task :api, [:version] => :environment do |t, args|
    unless args[:version].present? && args[:version].to_s.start_with?('v')
      raise "NO API VERSION GIVEN! usage: rails openapi_spec:api['v1']"
    end
    ...
    _route_set = Rails.application.routes.set.reject { |r| r.name.blank? }.map do |r|
      ...
      defaults.fetch(:noswagger, false) || %w[changelogs].include?(defaults['action'])
    end
    ...
    _routes.each do |route|
      begin
        ...
        Rails::Generators.invoke("openapi_spec", [route, "-f"])
      rescue => e
        puts '=' * 80
        puts " There was an error trying to open_speccer the route:\n #{route.white}".red
        puts '=' * 80
        puts ' Please check:'.red
        puts [Rails.root.to_s, :app, :controllers, "#{_route_set.dig(route, :controller)}_controller.rb"].join('/')
        puts '=' * 80
        raise e
      end
    end

    # processes the resulting endpoint path snippets with the api_v*.yaml and
    # creating the finalized OpenAPI spec yaml files
    OpenapiSpecGenerator.combine_specs!
  ...
  task :combine, [:version] => :environment do |t, args|
    # processes the resulting endpoint path snippets with the api_v*.yaml and
    # creating the finalized OpenAPI spec yaml files
    OpenapiSpecGenerator.combine_specs!

    puts '=' * 80
    puts " Finished combining!".green
    puts '=' * 80
